...
switch ($command) {
    ...
    deploy {
        ...
        git init
        git add -A
        git-push "CI: Deploy Med-GPT $new" --force
    }

    prod-test {
        git add -A
        git commit -S -m "Production test $new"
    ...
    dev-push {
        git checkout -b dev
        git add -A
        git commit -S -m "DEV: Med-GPT $new"
        git push origin dev $_force
        git checkout $branch
    }

    prod-push {
        if ($new -eq $old -and -not $_force) { 
            Write-Error "Versions are the same! Please bump."
            break 
        }
        
        git add -A
        git commit -S -m "PROD: Med-GPT $new"
    ...
    publish {
        ...
        Write-Host Releasing $new
        gh release delete $new -y
